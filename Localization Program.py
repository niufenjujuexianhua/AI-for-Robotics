# -*- coding: utf-8 -*-
"""
Created on Sun Jan  1 21:17:25 2017
...
def move(p,motions,p_move):
    q = [[0 for i in range(len(p[0]))] for k in range(len(p))]
    for row in range(len(q)):
        for col in range(len(q[0])):

#           moves up and down
            if motions[0] != 0:
                s = p_move * p[(row-motions[0]) % len(q)][col]
                q[row][col] = s + p[row][col]*(1-p_move)
#           moves left and right
            elif motions[1] != 0:
                s = p_move * p[row][(col-motions[1]) % len(p[0])]
                q[row][col] = s + p[row][col]*(1-p_move)
#            no motions
            elif motions == [0, 0]:
                s = p_move * p[row][col]
...
def sense(p,colors,measurements,sensor_right):  
    ...
    for row in range(len(q)):
        for col in range(len(q[0])):
            hit = (colors[row][col] == measurements)
