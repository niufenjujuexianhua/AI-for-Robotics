# -*- coding: utf-8 -*-
"""
Created on Sun Jan 22 22:03:35 2017
...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function    
        ...
        if beta >= 0.001: 
            ...
            x_prime = cx + sin(self.orientation+beta)*radius
            y_prime = cy - cos(self.orientation+beta)*radius
            theta = (self.orientation + beta)%(2*pi)
            
        else:          
            x_prime = self.x + motion[1]*cos(self.orientation)
            y_prime = self.y + motion[1]*sin(self.orientation)
            theta = self.orientation + beta
        
        self.set(x_prime, y_prime, theta)
        result = self
